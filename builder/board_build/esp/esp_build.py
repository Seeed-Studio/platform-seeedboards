...
#
# Attribution:
# This ESP32 family build script (adapted from the upstream pioarduino
# builder/main.py into builder/board_build/esp/) is based in part on
# pioarduino project work:
...
# Configure the ESP Python environment (penv) through builder/penv_setup.py.
# Upstream calls platform.setup_python_env(env) here because its platform.py
# owns the penv; this platform keeps platform.py a thin routing shim, so the
# SCons-side entry point of penv_setup.py is used directly instead.
# Must happen before importing penv-installed packages (fatfs, littlefs, etc.)
_penv_setup_file = platform_dir / "builder" / "penv_setup.py"
_penv_spec = importlib.util.spec_from_file_location("penv_setup", str(_penv_setup_file))
_penv_setup = importlib.util.module_from_spec(_penv_spec)
sys.modules["penv_setup"] = _penv_setup
_penv_spec.loader.exec_module(_penv_setup)
PYTHON_EXE, esptool_binary_path = _penv_setup.setup_python_environment(
    env, platform, str(core_dir)
)
...
# Import GDB_TOOL_PACKAGES from penv_setup loaded above
