...
#
# Attribution:
# This ESP32 family Arduino script (adapted from the upstream pioarduino
# builder/frameworks/arduino.py into builder/board_build/esp/) is based in
# part on pioarduino project work:
...
# component_manager.py lives under builder/frameworks/, not next to this
# script (builder/board_build/esp/); SCons only puts this script's own
# directory on sys.path (and strips __file__), so resolve the frameworks
# directory from the platform root before importing.
sys.path.insert(
    0,
    str(Path(DefaultEnvironment().PioPlatform().get_dir()) / "builder" / "frameworks"),
)
