...
3. Throughout active development and fork-based validation, keep the target sample's `platformio.ini` `platform` entry pointed at the declared fork, pinned to the exact pushed revision of the declared branch. Keep this test-only override and any local absolute path uncommitted. Do not restore the upstream platform URL merely because a validation build succeeds; restore it only when the developer explicitly starts upstream pull-request preparation.
