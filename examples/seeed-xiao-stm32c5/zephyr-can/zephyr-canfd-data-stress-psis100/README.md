# XIAO STM32C5 CAN FD Data-Phase Stress Sample (PSIS 100 MHz)
...
This variant selects PSIS as the FDCAN kernel-clock source and configures it at
100 MHz. Use this project when the test matrix explicitly requires the 100 MHz
clock configuration. Confirm the active clock with `cfd clock` before recording
results; timing values from a different FDCAN kernel clock are not equivalent.
...
```powershell
cd examples\seeed-xiao-stm32c5\zephyr-can\zephyr-canfd-data-stress-psis100
pio run
```
